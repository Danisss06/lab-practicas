...
import angulosComplementariosSuplementarios from "./suplementarioscomplementarios.json";
...
const AngulosComSupPage: React.FC = () => {
  ...
  const renderImage = (image: string) => {
    ...
    return (
      <img
        src={image}
        alt="example"
        className="w-full h-full object-cover mt-4 rounded-2xl justify-self-center"
      />
    );
  };
  ...
  return (
    <>
      ...
      <p className="mt-4">
        Los ángulos complementarios son los que al sumarse dan 90°.
      </p>
      {renderImage(imageURL[0])}
      <p className="mt-4">
        Los ángulos suplementarios son los que al sumarse dan 180°.
      </p>
      ...
      <div className="w-48 justify-self-center">{renderImage(imageURL[2])}</div>
...
export default AngulosComSupPage;
