...
const AnguloPerigonalPerigonoPage: React.FC = () => {
  ...
  const renderImage = (image: string) => {
    ...
    return (
      <img
        src={image}
        alt="example"
        className="w-40 h-full object-cover mt-4 rounded-2xl justify-self-center"
      />
    );
  };
  ...
  return (
    <>
      ...
      <p className="mt-4">
        Un ángulo periogonal o perigono es aquel que mide 360°.
      </p>
      {renderImage(imageURL[1])}
      <p className="mt-4 font-bold text-green-500">Ejemplo</p>
      <p className="mt-4">
        <span className="font-bold">Instrucciones:</span> considere la siguiente
        figura, la cual no está a escala, al formular su respuesta.
      </p>
...
export default AnguloPerigonalPerigonoPage;
